...
use pinoox\app\com_pinoox_insurance\model\GroupModel;
...
class UserModel extends AppDatabase
{
    public static function fetch_by_id($user_id)
    {
        self::$db->join(self::user_app . ' ua', 'u.user_id=ua.user_id', 'LEFT');
        self::$db->where('u.user_id', $user_id);
        return self::$db->getOne(self::user . ' u', 'u.*,ua.*,u.user_id, CONCAT_WS(" ", u.fname, u.lname) full_name');
    }
    ...
    public static function fetch_all($limit = null, $isCount = false, $checkApp = true)
    {
        ...
        self::$db->join(self::user_app . ' ua', 'u.user_id=ua.user_id', 'LEFT');
        $result = self::$db->get(self::user . ' u', $limit, 'u.*,ua.*,u.user_id, CONCAT_WS(" ", u.fname, u.lname) full_name');
    ...
    public static function uadate($form)
    {
    ...
    public static function uadate_self($form)
    {
        self::$db->where('user_id', $form['user_id']);
        return self::$db->update(self::user_app, [
            'group_key' => isset($form['group_key']) ? $form['group_key'] : GroupModel::getDefault(),
        ]);
    }

    public static function uadate_group_users($old_group_key,$new_group_key)
    {
    ...
    {
        return self::$db->insert(self::user_app, [
    ...
    public static function fetch_self_by_id($user_id)
    {
        ...
        return self::$db->getOne(self::user_app);
    }
    ...
    public static function uadate_setting($user_id, $data)
    {
        self::$db->where('user_id', $user_id);
        return self::$db->update(self::user_app, [
